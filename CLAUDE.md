...
`dev`/`build`/`check` each run `words:generate` first (via `predev`/`prebuild`/`precheck`), which regenerates the dictionary from SCOWL into `.cache/scowl/` — see [src/lib/dictionary.ts](src/lib/dictionary.ts). The first run needs network access and `make`, and takes roughly a minute; subsequent runs reuse the cached download/build and are fast.
