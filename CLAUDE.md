...
There is no test suite and no linter/formatter configured — `npm run check` is the only correctness gate. Always run it after making changes.
...
- [src/lib/SelectionDisplay.svelte](src/lib/SelectionDisplay.svelte) / [src/lib/TitleBar.svelte](src/lib/TitleBar.svelte) / [src/lib/Sidebar.svelte](src/lib/Sidebar.svelte) — presentational components driven entirely by props from `App.svelte`.
